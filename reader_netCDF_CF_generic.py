...
class Reader(BaseReader, StructuredReader):
    ...
    def __init__(self, filename=None, name=None, proj4=None, standard_name_mapping={}):
        ...
        if isinstance(filename, xr.core.dataset.Dataset): # PILOT 4: BELGIUM (DIEGO)
            self.Dataset = filename # PILOT 4: BELGIUM (DIEGO)
            self.name = name # PILOT 4: BELGIUM (DIEGO)
        else: # PILOT 4: BELGIUM (DIEGO)
        ...
        for var_name in self.Dataset.variables:
            ...
            if standard_name == 'time' or axis == 'T' or var_name in ['time', 'vtime']:
                ...
                elif isinstance(time[0], np.datetime64): # PILOT 4: BELGIUM (DIEGO)
                    from pandas import Timestamp # PILOT 4: BELGIUM (DIEGO)
                    Time = [Timestamp(i).to_pydatetime() for i in time] # PILOT 4: BELGIUM (DIEGO)
                    self.times = np.array(Time) # PILOT 4: BELGIUM (DIEGO)
                elif time.ndim == 2:
        ...
        if 'x' not in locals():  # No x/y-coordinates were detected
            ...
            if lon_var.ndim == 1:
                ...
                #x = lon_var.data # DIEGO
                #y = lat_var.data # DIEGO 
